...
| `image_path` | string | 图像模式下必填 | 图像文件本地路径，作为图像模态输入 |
...
- 若当前阶段没有与样本逐条对应的图像，可将 `train.image_column` 设为 `null`，此时 `image_path` 可以不提供。
...
若暂时没有语音数据，可关闭语音列配置。若图像也无法与样本逐条对应，可同时关闭图像列，先使用文本 + 结构化双模态。
