...
class MultiModalEvaluator(nn.Module):
    def __init__(self, config: AICTConfig, tabular_dim: int) -> None:
        super().__init__()
        self.config = config
        self.use_image = bool(config.train.image_column)
        self.use_audio = bool(config.train.audio_column)
        self.modality_names = ["text"]
        if self.use_image:
            self.modality_names.append("image")
        if self.use_audio:
    ...
    def forward(
        ...
        feature_tokens = [text_features]
        if self.use_image:
            if image is None:
                raise ValueError("当前模型已启用图像模态，但未收到 image 输入。")
            ...
            feature_tokens.append(self.image_projector(image_encoded))
        if self.use_audio:
